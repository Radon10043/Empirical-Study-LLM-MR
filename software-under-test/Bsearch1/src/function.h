...
int BinSearch(int i_array[], int x, int i_start, int i_end) {
    ...
    if (i_start > i_end)
        ...
        if (i_array[i_mid] == x)
            return (i_mid);
        if (i_array[i_mid] > x)
            return BinSearch(i_array, x, i_start, i_mid - 1);
        else
            return BinSearch(i_array, x, i_mid + 1, i_end);
    }
}
...
vector<int> get_range(vector<int> vec, int x) {
    int index = BinSearch(vec.data(), x, 0, vec.size() - 1);
    return GetRange(vec.data(), index, vec.size() - 1);
}
