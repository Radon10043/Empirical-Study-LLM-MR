...
class TestingClass(unittest.TestCase):
    ...
    def test25(self, pattern: str, file: str):
        """Metamorphic Relation 25: If the pattern is changed to its zero-width positive lookahead form, the output should remain the same."""
        # Get source output
        process_orig = os.popen(f"{GREP_PATH} '{pattern}' {file}")
        source_out = process_orig.readlines()
        process_orig.close()

        # Construct follow-up input with the pattern in zero-width positive lookahead form
