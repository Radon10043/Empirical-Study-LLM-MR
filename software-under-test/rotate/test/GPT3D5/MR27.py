...
class TestingClass(unittest.TestCase):
    ...
    def test27(self, img: np.array, angle: float):
        """Metamorphic Relation 27: Rotating the image by angle A and then applying a laplace filter to the resulting image is equivalent to applying the laplace filter to the original image and then rotating the result by angle A."""
        # Get source output
