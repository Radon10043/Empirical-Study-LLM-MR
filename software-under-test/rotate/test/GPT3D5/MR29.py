...
class TestingClass(unittest.TestCase):
    ...
    def test29(self, img: np.array, angle: float):
        """Metamorphic Relation 29: Rotating the image by angle A and then applying a median filter to the resulting image is equivalent to applying the median filter to the original image and then rotating the result by angle A."""
        # Get source output
