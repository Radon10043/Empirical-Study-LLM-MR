...
class TestingClass(unittest.TestCase):
    ...
    def test34(self, img: np.array, angle: float):
        """Metamorphic Relation 34: Rotating the image by angle A and then applying a uniform filter to the resulting image is equivalent to applying the uniform filter to the original image and then rotating the result by angle A."""
        # Get source output
