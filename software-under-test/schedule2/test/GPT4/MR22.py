import os, sys

sys.path.append(os.path.join(os.path.dirname(__file__), ".."))
from utils import *


class TestingClass(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        """Build schedule2.out before starting testing."""
        cmd = "cd " + os.path.join(os.path.dirname(__file__), "..", "..", "src") + " && make clean all"
        proc = os.popen(cmd)
        proc.readlines()
        proc.close()

    @parameterized.expand(load_test_cases(1000))
    def test22(self, job_list: list): # Fixed
        """Metamorphic Relation 22: For every process in the queue, increase the priority to max and then decrease it to
        the minimum, repeating the operation, and the final output order should be the same as the initial."""
        cmd_list = [SCHEDULE_PATH]
        cmd_list.extend(job_list)

        source_op = SCHEDULE_OPERATIONS["FLUSH"]
        source_out = subprocess.check_output(cmd_list, input=source_op, text=True).split("\n")

        # Increase priority to MAXPRIO and decrease to MINPRIO for all processes
        follow_op = str()
        job_cnt = int(job_list[0]) + int(job_list[1]) + int(job_list[2])
        for _ in range(job_cnt):  # Assuming the number of initial conditions matches the number of jobs added
            follow_op += f"{SCHEDULE_OPERATIONS['UPGRADE_PRIO']} {PRIORITY_LEVEL['LOW']} 1.0\n"  # Upgrade to max
            follow_op += f"{SCHEDULE_OPERATIONS['UPGRADE_PRIO']} {PRIORITY_LEVEL['HIGH']} 0.0\n"  # Downgrade to min
        follow_op += SCHEDULE_OPERATIONS["FLUSH"]

        # Save the output order after priority changes
        follow_out = subprocess.check_output(cmd_list, input=follow_op, text=True).split("\n")

        # Verify that the initial order is the same as after the upgrades and downgrades
        self.assertEqual(source_out, follow_out)


if __name__ == "__main__":
    unittest.main()
