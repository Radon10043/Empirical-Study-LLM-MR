...
Based on the above case, please identify the metamorphic relation of the program `print_tokens`. `print_tokens` is a lexical analyzer from Simens programs. Its inputs are a sequence of strings, and its output is all the tokens and their categories in order.

Please identify the metamorphic relations of `print_tokens` as much as possible and codify them as Python code. Here is an example:
